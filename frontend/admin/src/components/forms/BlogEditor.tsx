...
export default function BlogEditor({
  ...
  const onSubmit = async (values: z.infer<typeof schema>) => {
    try {
      ...
      onSaved?.();
      window.dispatchEvent(new Event("roxbusi:blog-changed"));
      closeRef.current?.click();
    } catch (error) {
      toast.error(
        error instanceof ApiError ? error.message : "Không thể lưu bài viết.",
      );
    }
  };
    ...
    <SheetContent className="overflow-y-auto">
      <ScrollArea className="h-screen pr-4">
        <SheetHeader>
          ...
          <SheetDescription>
            Nội dung hỗ trợ các thẻ HTML định dạng cơ bản; mã nguy hiểm sẽ bị
            máy chủ loại bỏ.
          </SheetDescription>
        </SheetHeader>
        {loadingPost ? (
          ...
          <Form {...form}>
            ...
            >
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} placeholder="Cẩm nang phong cách" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Đường dẫn ảnh bìa hiện tại</FormLabel>
                    <FormControl>
              ...
              />
              <FormItem>
                <FormLabel>Chọn ảnh bìa mới</FormLabel>
                <FormControl>
                  <Input
                    type="file"
